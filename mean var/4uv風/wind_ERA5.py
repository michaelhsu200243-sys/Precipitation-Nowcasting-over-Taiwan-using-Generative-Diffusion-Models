...
@author: user
Description: 整合風速數據讀取、2015-2021年份篩選、96x96插值、統計量計算與NPZ更新。
"""
...
def load_filter_and_process_wind_data(data_dir, file_pattern, start_year, end_year):
    ...
    # --- 步驟 2：篩選年份 ---
    print("\n⏳ 開始根據 'Time' 欄位篩選 2015 至 2021 年的數據...")
    start_date = pd.to_datetime(f'{start_year}-01-01 00:00:00')
    end_date = pd.to_datetime(f'{end_year+1}-01-01 00:00:00')
    
    df_filtered = df_combined[(df_combined['Time'] >= start_date) & (df_combined['Time'] < end_date)].copy()
    
    if df_filtered.empty:
        print("錯誤：篩選後沒有找到 2015-2021 年的數據。")
        sys.exit()
...
u_csv_path = os.path.join(output_dir, 'u_wind_processed_96x96_2015-2021.csv')
v_csv_path = os.path.join(output_dir, 'v_wind_processed_96x96_2015-2021.csv')
